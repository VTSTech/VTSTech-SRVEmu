# buddy_system_r08.py - COMPLETE BUDDY API IMPLEMENTATION
import time
...
class BuddyHandlers:
    ...
    def handle_buddy_command(self, data, session):
        """Main buddy command dispatcher"""
        data_str = data.decode('latin1') if data else ""
        print(f"BUDDY API: Command data: {data_str}")
        
        # Parse command type from first 4 bytes
        if len(data_str) >= 4:
            command = data_str[:4]
            params = data_str[4:] if len(data_str) > 4 else ""
        else:
            command = "UNKN"
            params = data_str
        ...
        if command in handlers:
            return handlers[command](params, session)
        else:
            print(f"BUDDY API: Unknown command: {command}")
            return self.create_packet(command, '', "STATUS=0\nERROR=Unknown command\n")
